...
export class Map extends Evented {
  initialize(id, options) {
    ...
    if (options.center && options.zoom !== undefined) {
      this.setView(new LatLng(options.center), options.zoom, {
        reset: true
      });
    }
    this.callInitHooks();
  ...
  _initEvents(remove) {
    ...
    onOff(this._container, 'click dblclick pointerdown pointerup ' + 'pointerover pointerout pointermove contextmenu keypress keydown keyup', this._handleDOMEvent, this);
  ...
  static _pointerEvents = (this.setDefaultOptions({
    ...
    trackResize: true
  }), ['click', 'dblclick', 'pointerover', 'pointerout', 'contextmenu']);
  _fireDOMEvent(e, type, canvasTargets) {
    ...
    if (e.type !== 'keypress' && e.type !== 'keydown' && e.type !== 'keyup') {
      ...
      data.containerPoint = isMarker ? this.latLngToContainerPoint(target.getLatLng()) : this.pointerEventToContainerPoint(e);
      data.layerPoint = this.containerPointToLayerPoint(data.containerPoint);
      data.latlng = isMarker ? target.getLatLng() : this.layerPointToLatLng(data.layerPoint);
    }
    for (const t of targets) {
      t.fire(type, data, true);
      if (data.originalEvent._stopped || t.options.bubblingPointerEvents === false && Map._pointerEvents.includes(type)) {
        return;
      }
    }
  }
  ...
  _getTopLeftPoint(center, zoom) {
    const pixelOrigin = center && zoom !== undefined ? this._getNewPixelOrigin(center, zoom) : this.getPixelOrigin();
  ...
  _latLngBoundsToNewLayerBounds(latLngBounds, zoom, center) {
    const topLeft = this._getNewPixelOrigin(center, zoom);
    return new Bounds([this.project(latLngBounds.getSouthWest(), zoom)._subtract(topLeft), this.project(latLngBounds.getNorthWest(), zoom)._subtract(topLeft), this.project(latLngBounds.getSouthEast(), zoom)._subtract(topLeft), this.project(latLngBounds.getNorthEast(), zoom)._subtract(topLeft)]);
  }
  ...
  _getBoundsOffset(pxBounds, maxBounds, zoom) {
    const projectedMaxBounds = new Bounds(this.project(maxBounds.getNorthEast(), zoom), this.project(maxBounds.getSouthWest(), zoom)),
