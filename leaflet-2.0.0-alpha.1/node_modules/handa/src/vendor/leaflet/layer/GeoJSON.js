...
export function coordsToLatLngs(coords, levelsDeep, _coordsToLatLng) {
  return coords.map(coord => levelsDeep ? coordsToLatLngs(coord, levelsDeep - 1, _coordsToLatLng) : (_coordsToLatLng || coordsToLatLng)(coord));
}
...
export function latLngsToCoords(latlngs, levelsDeep, close, precision) {
  ...
  const coords = latlngs.map(latlng => levelsDeep ? latLngsToCoords(latlng, LineUtil.isFlat(latlng) ? 0 : levelsDeep - 1, close, precision) : latLngToCoords(latlng, precision));
